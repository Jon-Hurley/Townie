<script>
	...
	$: colorStyle = [redStyle, greenStyle, grayStyle, amberStyle][status];
	$: color = ['red', 'green', 'gray', 'amber'][status];
	$: name = ['Error', 'Success', 'Confirm', 'Warning'][status];
	$: bStyle = `${buttonStyle} ${colorStyle}`;
	$: svg = [
