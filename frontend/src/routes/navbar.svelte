...
{#if !['/login', '/signup'].includes(currentPage)}
    <nav class="absolute bottom-0 bg-gray-800 w-full px-2 h-14 flex items-center justify-around overflow-hidden">
        {#each pages as { page, svg }}
            <button
                class="rounded-full p-8 text-gray-400 hover:text-white transition-colors duration-500 {page === currentPage ? 'bg-indigo-700' : 'hover:bg-indigo-700'}"
                on:click={() => {
                    if (page === '/login') {
                        // logout function
                    }
