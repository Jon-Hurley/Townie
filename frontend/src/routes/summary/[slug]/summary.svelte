<script>
	...
	const _submitRating = async (userRating) => {
		if (
			...
			alert('Please select a valid rating to submit.');
			return;
		}
		userRating = parseInt(userRating);
		console.log(userRating + ' ' + ratings.rating + ' ' + ratings.numRatings);
	...
	let media = [
		{
			...
			link:
				'https://twitter.com/intent/tweet?text=Check%20out%20this%20Townie%20game%20I%20played%3A%0A' +
		...
		},
		{
			...
			link:
				'https://www.reddit.com/submit?&title=Check%20out%20this%20Townie%20game%20I%20played%3A%0A&url=' +
				window.location.href
		},
		{
			name: 'Mail',
			icon: 'M 82.599 78.237 H 7.401 C 3.32 78.237 0 74.918 0 70.837 V 19.164 c 0 -4.081 3.32 -7.401 7.401 -7.401 h 75.198 c 4.081 0 7.401 3.32 7.401 7.401 v 51.673 C 90 74.918 86.68 78.237 82.599 78.237 z M 7.401 13.762 C 4.423 13.762 2 16.185 2 19.164 v 51.673 c 0 2.978 2.423 5.4 5.401 5.4 h 75.198 c 2.979 0 5.401 -2.423 5.401 -5.4 V 19.164 c 0 -2.979 -2.423 -5.401 -5.401 -5.401 H 7.401 z',
			link:
				'mailto:?&body=Check%20out%20this%20Townie%20game%20I%20played%3A%0Alink' +
				window.location.href
		},
		{
			name: 'Whatsapp',
			...
			link:
				'https://api.whatsapp.com/send/?text=Check%20out%20this%20Townie%20game%20I%20played%3A%0A' +
				window.location.href +
				'&type=custom_url'
		},
		{
