...
{#if user.key !== $userStore.key}
	<div class="flex justify-center flex-wrap">
		<button disabled class="{indigoStyle} {buttonStyle} m-1"> Message </button>

		{#if !user?.friendship?.length}
			<button class="{blueStyle} {buttonStyle} m-1" on:click={_sendFriendRequest}>
				Send Friend Request
			</button>
		{:else if user.friendship[0].status}
			<button class="{redStyle} {buttonStyle} m-1" on:click={_rejectFriend}> Remove Friend </button>
		{:else if user.friendship[0].inbound}
			<button class="{greenStyle} {buttonStyle} m-1" on:click={_acceptFriend}>
				Accept Friend Request
			</button>
			<button class="{redStyle} {buttonStyle} m-1" on:click={_rejectFriend}>
		...
		{#each user.purchases as p}
			...
			>
				<svg
					...
					stroke="currentColor"
				>
					<path stroke-linecap="round" stroke-linejoin="round" d={p.badge} />
				</svg>
